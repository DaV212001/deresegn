...
class InvoicePdfService {
  ...
  static Future<void> generateIntoDocument(
    ...
    final docNumber =
        overrideDocNumber ??
        _n(docDet['DocumentNumber'] ?? invoice.documentNumber);
    final docDate = _n(docDet['Date'] ?? invoice.createdAt);
    final txType = _n(
      payload['TransactionType'] ?? invoice.transactionType,
      'B2C',
    );
    ...
    final discountValue = _fmt(
      valDet['Discount'] ?? invoice.totals.discount ?? '0.00',
    );
    final exciseValue = _fmt(
      valDet['ExciseValue'] ?? invoice.totals.exciseValue ?? '0.00',
    );
    final incomeWithholdValue = valDet['IncomeWithholdValue'] ?? '0.00';
    final txnWithholdValue = valDet['TransactionWithholdValue'] ?? '0.00';
    final grandTotal =
        double.tryParse(
          (valDet['TotalValue'] ?? invoice.totals.totalValue ?? '0').toString(),
        ) ??
        0.0;
    ...
    pw.Widget kv(String label, String value) => pw.Padding(
      ...
      child: pw.Row(
        ...
        children: [
          pw.SizedBox(width: 90, child: pw.Text(label, style: styleSmall)),
    ...
    pw.Widget summaryRow(String label, String value, {bool bold = false}) =>
        pw.Padding(
          ...
          child: pw.Row(
            ...
            children: [
              pw.Text(label, style: bold ? styleBold : styleBody),
              pw.Text(
                value,
                style: bold
                    ? pw.TextStyle(
                  ...
                  child: pw.Container(
                    ...
                    child: pw.Column(
                      ...
                      children: [
                        ...
                        kv('ከተማ / City', _n(seller['City'])),
                        kv('ቀበሌ / Kebele', _n(seller['Kebele'])),
                        kv(
                          'ዞን/ክፍለ ከተማ / Zone',
                          _n(seller['Zone'] ?? seller['Wereda']),
                        ),
                        kv('የተ.ኢ.ታ ቁጥር / VAT Reg.No', _n(seller['VatNumber'])),
                        kv('የማብር መለያ ቁጥር / TIN', _n(seller['Tin'])),
                        kv('ስ.ቁ / Phone', _n(seller['Phone'])),
                        kv('ኢሜይል / Email', _n(seller['Email'])),
                      ],
                    ),
                  ),
                ),
                  ...
                  child: pw.Container(
                    ...
                    child: pw.Column(
                      ...
                      children: [
                        ...
                        pw.Text(
                          _n(
                            buyer['LegalName'] ?? invoice.buyer.legalName,
                            'Customer',
                          ),
                          style: pw.TextStyle(
                            fontSize: 10,
                        ...
                        kv('ቀበሌ / Kebele', _n(buyer['Kebele'])),
                        kv(
                          'ዞን/ክፍለ ከተማ / Zone',
                          _n(buyer['Zone'] ?? buyer['Wereda']),
                        ),
                        kv('የተ.ኢ.ታ ቁጥር / VAT Reg.No', _n(buyer['VatNumber'])),
                        kv(
                          'የማብር መለያ ቁጥር / TIN',
          ...
          if (docType != 'TWTH')
                  ...
                  child: pw.Container(
                    ...
                    child: pw.Column(
                      children: [
                        ...
                        summaryRow('ተ.ኢ.ታ / VAT 15%', '$taxValue ETB'),
                        if (double.tryParse(incomeWithholdValue.toString()) !=
                                null &&
                            double.parse(incomeWithholdValue.toString()) > 0)
                          summaryRow(
                            'የገቢ ግብር ቅናሽ / Income Withhold',
                            '${_fmt(incomeWithholdValue)} ETB',
                          ),
                        if (double.tryParse(txnWithholdValue.toString()) !=
                                null &&
                            double.parse(txnWithholdValue.toString()) > 0)
                          summaryRow(
                            'የግብይት ግብር ቅናሽ / Txn Withhold',
                            '${_fmt(txnWithholdValue)} ETB',
                          ),
            ...
            pw.Container(
              ...
              child: pw.Column(
                ...
                children: [
                  pw.Text(
                    'Withholding Summary / የግብር ቅነሳ ማጠቃለያ',
                    style: styleBold,
                  ),
                  pw.SizedBox(height: 10),
                  summaryRow(
                    'Withheld Amount',
                    '${_fmt(payload['CollectedAmount'] ?? 0)} ETB',
                  ),
                ...
                pw.Expanded(
                  child: pw.Column(
                    ...
                    children: [
                      ...
                      pw.Text(
                        '$irnTitle $irn',
                        style: pw.TextStyle(fontSize: 6.5, color: bodyText),
                      ),
                      pw.SizedBox(height: 2),
                      pw.Text(
                        'Powered by ${_n('Micro Sun & Solution PLC')}',
                        style: pw.TextStyle(
                          fontSize: 7,
                          fontStyle: pw.FontStyle.italic,
  ...
  static pw.Widget _buildItemsTable(
    ...
    final dataRows = items.asMap().entries.map((entry) {
      ...
      pw.Widget cell(String v, {pw.TextAlign align = pw.TextAlign.left}) =>
          pw.Container(
